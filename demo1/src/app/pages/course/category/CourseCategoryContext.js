...
export const CourseCategoryContextProvider = ({children}) => {
  ...
  const getCourseCategoryLists = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/courses/categories', config)
  ...
  const createCourseCategoryMutation = useMutation({
    mutationFn: async (data) => {
      console.log(data)
      return axios
        .post('http://localhost:8080/api/courses/addCategory', data, config)
        .then((res) => res.data)
    },
    onMutate: () => {
      console.log('mutate')
  ...
  const deleteCourseCategoryMutation = useMutation({
    mutationFn: async (id) => {
      return axios
        .delete(`http://localhost:8080/api/courses/category/${id}`, config)
        .then((res) => res.data)
    },
    onSuccess: () => {
  ...
  const updateCourseCategoryMutation = useMutation({
    mutationFn: async (updateData) => {
      ...
      return axios
        .put(`http://localhost:8080/api/courses/category/${updateData._id}`, updateData, config) // Corrected order of arguments
        .then((res) => res.data)
    },
    onSettled: async (_, error) => {
      if (error) {
